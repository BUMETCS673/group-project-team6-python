...
class TestViewQuestion(TestCase):
    ...
    def setUp(self):
        ...
        self.instance = Instance.objects.create(instance_name="test 1", instructor=self.instructor, slug="test-1")
        # survey
        self.survey = Survey.objects.create(instance=self.instance, survey_name="test-survey")
        # add question url
        self.add_question_url = reverse('survey:add_question', args=[self.survey.survey_id])
    ...
    def test_add_question(self):
        ...
        response = self.instructor_login.post(
            path=self.add_question_url,
            data=data
        )
        # check in the database
        self.assertEqual(response.status_code, 302)
        self.assertEqual(len(Question.objects.filter(survey=self.survey)), 1)
